...
export default function GapNoAiDrivenRouteOptimization() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-driven-route-optimization/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-ai-driven-route-optimization', project: 'AIVendingMachineNetworkManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-driven-route-optimization/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No AI-driven route optimization</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIVendingMachineNetworkManager</p>
      </div>
