...
export default function CfDynamicPricingRecommendationsByDemandInventorySeasonality() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-dynamic-pricing-recommendations-by-demand-inventory-seasonality/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-dynamic-pricing-recommendations-by-demand-inventory-seasonality', project: 'AIVendingMachineNetworkManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-dynamic-pricing-recommendations-by-demand-inventory-seasonality/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Dynamic pricing recommendations by demand/inventory/seasonality</h1>
