...
// Feature: Predictive maintenance based on telemetry signatures
...
export default function CfPredictiveMaintenanceBasedOnTelemetrySignatures() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-maintenance-based-on-telemetry-signatures/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-predictive-maintenance-based-on-telemetry-signatures', project: 'AIVendingMachineNetworkManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-maintenance-based-on-telemetry-signatures/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Predictive maintenance based on telemetry signatures</h1>
