...
// Feature: AI demand forecaster by machine/location/time for restocking
...
export default function CfAiDemandForecasterByMachineLocationTime() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-ai-demand-forecaster-by-machine-location-time-for/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-ai-demand-forecaster-by-machine-location-time-for', project: 'AIVendingMachineNetworkManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-ai-demand-forecaster-by-machine-location-time-for/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">AI demand forecaster by machine/location/time for restocking</h1>
