...
// Feature: Route optimizer minimizing collection/restocking distance
...
export default function CfRouteOptimizerMinimizingCollectionRestockingDistance() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-route-optimizer-minimizing-collection-restocking-distance/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-route-optimizer-minimizing-collection-restocking-distance', project: 'AIVendingMachineNetworkManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-route-optimizer-minimizing-collection-restocking-distance/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Route optimizer minimizing collection/restocking distance</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVendingMachineNetworkManager</p>
      </div>
