...
// Theft detection identifying cash/inventory discrepancies
...
router.post('/analyze', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'theft-detection', payload, result);
    res.json({ success: true, feature: 'theft-detection', ...result });
  } catch (err) {
    console.error('theft-detection analyze error:', err.message);
...
router.post('/batch', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Theft detection identifying cash/inventory discrepancies. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'theft-detection', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authenticateToken, (req, res) => {
  res.json({ feature: 'theft-detection', title: 'Theft detection identifying cash/inventory discrepancies', project: 'AIVendingMachineNetworkManager' });
});
