...
// Cashless wallet/QR integration for modern vending
// Feature: cashless-integration
...
router.post('/analyze', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'cashless-integration', payload, result);
    res.json({ success: true, feature: 'cashless-integration', ...result });
  } catch (err) {
    console.error('cashless-integration analyze error:', err.message);
...
router.post('/batch', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Cashless wallet/QR integration for modern vending. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'cashless-integration', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authenticateToken, (req, res) => {
  res.json({ feature: 'cashless-integration', title: 'Cashless wallet/QR integration for modern vending', project: 'AIVendingMachineNetworkManager' });
});
