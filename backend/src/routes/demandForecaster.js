...
// AI demand forecaster by machine/location/time for restocking
// Feature: demand-forecaster
...
router.post('/analyze', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'demand-forecaster', payload, result);
    res.json({ success: true, feature: 'demand-forecaster', ...result });
  } catch (err) {
    console.error('demand-forecaster analyze error:', err.message);
...
router.post('/batch', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: AI demand forecaster by machine/location/time for restocking. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'demand-forecaster', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authenticateToken, (req, res) => {
  res.json({ feature: 'demand-forecaster', title: 'AI demand forecaster by machine/location/time for restocking', project: 'AIVendingMachineNetworkManager' });
});
