...
// Route optimizer minimizing collection/restocking distance
...
router.post('/analyze', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'route-optimizer', payload, result);
    res.json({ success: true, feature: 'route-optimizer', ...result });
  } catch (err) {
    console.error('route-optimizer analyze error:', err.message);
...
router.post('/batch', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Route optimizer minimizing collection/restocking distance. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'route-optimizer', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authenticateToken, (req, res) => {
  res.json({ feature: 'route-optimizer', title: 'Route optimizer minimizing collection/restocking distance', project: 'AIVendingMachineNetworkManager' });
});
